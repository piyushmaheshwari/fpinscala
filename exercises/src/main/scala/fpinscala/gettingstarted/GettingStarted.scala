...
object MyModule {
  ...
  def fib(n: Int): Int = {
    ...
    def go(a: Int, b: Int, count: Int): Int =
      if (n == count) b else go(b, a + b, count + 1)

    if (n == 0) 0 else if (n == 1) 1 else go(1, 1, 2)
  }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean = {
    @annotation.tailrec
    def go (array: Array [A], prev: A): Boolean = {
      if (array.isEmpty)
        true
      else {
        val head = array.head
        if (!gt (prev, head)) false else go (array.tail, head)
      }
    }
